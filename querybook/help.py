    ...
    for i in range(len(SplitDatasList)):
        if i == 0:
            ...
            AddKeyDict[SplitDatasList[i]] = str(ConvertToBeatifuleNumber(SplitDatasList[i - 1])) \
                                            + "<" + ZhiBiaoName + "<=" + str(ConvertToBeatifuleNumber(SplitDatasList[i]))
    return AddKeyDict
...
def ConvertRecommendNovelListToJsonFormat(NovelList):
    JsonNovelList = []
    for Novel in NovelList:
        NovelDict = {}
        NovelDict["pic_url"] = Novel[0]
        NovelDict["title"] = Novel[1]
        NovelDict["introduce"] = Novel[2]
        NovelDict["author"] = Novel[3]
        NovelDict["hyperlink"] = Novel[4]
        NovelDict["category"] = Novel[5]
        NovelDict["update_time"] = Novel[6]
        NovelDict["wordcount"] = Novel[7]
        JsonNovelList.append(NovelDict)
    return JsonNovelList

def MakeReturnJsonRecommendNovelListByThree(lis1,lis2,lis3):
    if len(lis1) >= 10:
        return lis1[0:10]
    elif len(lis2) >= (10-len(lis1)):
        return lis1 + lis2[0:(10-len(lis1))]
    elif len(lis3) >= (10-len(lis1)-len(lis2)):
        return lis1 + lis2 + lis3[0:(10-len(lis1)-len(lis2))]
    else:
